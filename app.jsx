...
function ScoreHistoryPanel({p, sfm, setSfm, addScore, delScore, addWelledLog, delWelledLog}){
  ...
  const miniCard = (key,title,ptsArr,accent)=>{
    ...
    const chartPoints = ptsArr.map((pt,i)=>({x:i,y:pctOf(pt)??0,label:pt.date}));
    return(
      <div key={key} style={{background:"#fff",borderRadius:10,padding:11,border:`1.5px solid ${accent}25`,borderLeft:`4px solid ${accent}`}}>
        <div style={{fontSize:11,fontWeight:800,color:"#334155",marginBottom:5,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}} title={title}>{title}</div>
        ...
        <div style={{fontSize:9,color:"#94a3b8",marginTop:4}}>{ptsArr.length} pt{ptsArr.length!==1?"s":""} · {last.date}</div>
      </div>
    );
  };
