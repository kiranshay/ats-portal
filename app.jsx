...
// Session 18C v33: tolerant catalog lookup. The bug Jackson hit ("No
// bubble sheet available — type your answers below" on worksheets that
// DO exist in the catalog) was an exact-match failure: the worksheet
// title stored on the assignment differed from the catalog title by
// something invisible — a trailing/doubled space, a non-breaking space
// ( ), a zero-width char, a unicode dash (–/—) vs hyphen, or case.
// catalog.find(c => c.title === w.title) then returned undefined and the
// editor dropped to the plain-textarea fallback, so the student had no
// questions to answer.
//
// normalizeTitle collapses all of those so matching is robust. We still
// prefer an exact match first (fast, zero risk), then fall back to a
// normalized match, then to a normalized match ignoring the "(NQs)"
// suffix. Difficulty (Easy/Medium/Hard/Comprehensive) is preserved by
// normalization, so we never cross-match difficulties.
function normalizeTitle(s){
  // Pure-ASCII normalization: lowercase, then collapse ANY run of
  // non-alphanumeric chars (spaces, hyphens/unicode dashes, parens,
  // non-breaking/zero-width spaces, punctuation) into a single space.
  // This makes title matching immune to invisible-character and
  // dash/whitespace differences without needing unicode char classes.
  return (s == null ? "" : String(s))
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, " ")
    .trim();
}
function findCatalogEntry(catalog, title){
  if(!Array.isArray(catalog) || !title) return null;
  // 1. Exact match - unchanged fast path.
  let row = catalog.find(c => c && c.title === title);
  if(row) return row;
  // 2. Normalized match - tolerates whitespace/dash/case/invisible chars.
  const want = normalizeTitle(title);
  row = catalog.find(c => c && normalizeTitle(c.title) === want);
  if(row) return row;
  // 3. Suffix-insensitive match - tolerate a missing/extra "(NQs)" suffix
  //    (after normalization "(5Qs)" becomes "5qs" at the end).
  const stripQs = (t) => normalizeTitle(t).replace(/\s*\d+\s*qs?$/, "").trim();
  const wantNoQs = stripQs(title);
  if(wantNoQs){
    row = catalog.find(c => c && stripQs(c.title) === wantNoQs);
    if(row) return row;
  }
  return null;
}
...
function StudentPortal({studentId, onSignOut, currentUserEntry, switcherSlot, impersonating}){
  ...
  return (
    <PortalShell studentName={student.name} studentGrade={student.grade} onSignOut={onSignOut} currentUserEntry={currentUserEntry} switcherSlot={switcherSlot} impersonating={impersonating} studentEmail={(student.meta && student.meta.email) || (currentUserEntry && currentUserEntry.email) || ""}>
      ...
      {tab==="history"  && <PortalHistoryTab student={student} studentId={studentId} currentUserEntry={currentUserEntry} deepLinkAssignmentId={deepLinkAssignmentId} submissions={portalSubmissions} impersonating={impersonating}/>}
...
function SingleWorksheetEditor({studentId, assignment, worksheetId, readOnly, onClose}){
  ...
  const catalogEntry = useMemo(()=>{
    ...
    // Session 18C v33: tolerant lookup so an invisible-char / dash /
    // whitespace difference in the stored title can't strand a worksheet
    // on the "no bubble sheet" textarea fallback (Jackson's bug).
    const row = findCatalogEntry(catalog, worksheet.title);
    if((!row || !Array.isArray(row.questionIds) || row.questionIds.length === 0)){
      console.warn("[catalog] no questionIds match for worksheet title:", JSON.stringify(worksheet.title));
    }
...
function SubmissionEditor({studentId, assignment, readOnly, onClose, focusWorksheetId}){
  ...
  const catalogByWorksheetId = useMemo(()=>{
    ...
    for(const w of worksheetsStable){
      const entry = findCatalogEntry(catalog, w.title);
      ...
      {open && (
        <div style={{marginTop:10,display:"flex",flexDirection:"column",gap:10}}>
          {worksheets.map(w=>{
            ...
            const catalogEntry = findCatalogEntry(wsCatalog || [], w.title);
            return (
...
function TutorPerWsWorksheetCard({studentId, assignmentId, worksheet, doc, statusStyle, catalogEntry, regradeBusy, regradeResult, onRegrade}){
  ...
  // Session 18C v39: tutor can edit the student's per-question answers
  // (e.g. the student answered on paper or mis-entered). Saving writes the
  // corrected responses back to the per-WS doc and immediately re-grades
  // them via the existing regrade callable, so the score + chips update.
  const [editing, setEditing] = useState(false);
  const [editAns, setEditAns] = useState({});   // questionIndex -> string
  const [savingEdit, setSavingEdit] = useState(false);

  const startEdit = () => {
    const init = {};
    for(const s of slots){
      init[s.i] = (s.r && typeof s.r.studentAnswer === "string") ? s.r.studentAnswer : "";
    }
    setEditAns(init);
    setEditing(true);
  };
  const saveEdits = async () => {
    ...
    try{
      const col = studentAssignmentWorksheetSubmissionsCollection(studentId, assignmentId);
      if(!col) throw new Error("Firestore not ready");
      // Rebuild the full responses array, preserving each question's
      // existing flag + usedDesmos, overriding only studentAnswer.
      const byIdx = new Map(responses.map(r => [Number(r.questionIndex), r]));
      const len = Math.max(slots.length, qIds.length, responses.length);
      const newResponses = [];
      for(let i = 0; i < len; i++){
        ...
        const obj = {
          questionIndex: i,
          studentAnswer: typeof v === "string" ? v : (prev.studentAnswer || ""),
        };
        if(prev.flag) obj.flag = prev.flag;
        if(prev.usedDesmos === true) obj.usedDesmos = true;
        newResponses.push(obj);
      }
      const FV = firebase.firestore.FieldValue;
      await col.doc(worksheet.id).set({
        responses: newResponses,
        tutorEditedAt: FV.serverTimestamp(),
        updatedAt: FV.serverTimestamp(),
      }, {merge: true});
      setEditing(false);
      // Re-grade with the corrected answers (regrade reads the doc fresh).
      if(onRegrade) await onRegrade();
    } catch(e){
      console.warn("[tutor-edit] save failed:", e);
      alert("Could not save edited answers: " + (e.message || e));
    } finally {
      setSavingEdit(false);
    }
  };
  ...
  const canEditAnswers = doc && (isSubmitted || isGraded) && slots.length > 0;
  ...
  return (
    <div style={{padding:"12px 14px",background:"#fff",border:`1px solid ${statusStyle.border}`,borderRadius:6}}>
      ...
      {/* Edit answers (tutor override) */}
      {editing && (
        <div style={{marginTop:8,marginBottom:10,padding:"10px 12px",background:"#F4F8FB",border:"1px solid rgba(0,50,88,.25)",borderRadius:6}}>
          <div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:9,fontWeight:700,letterSpacing:1.2,color:"#003258",textTransform:"uppercase",marginBottom:8}}>
            Edit student answers — saving re-grades
          </div>
          <div style={{display:"flex",flexWrap:"wrap",gap:"8px 12px"}}>
            {slots.map(({i})=>(
              <label key={i} style={{display:"inline-flex",alignItems:"center",gap:4,fontFamily:"'IBM Plex Mono',monospace",fontSize:12}}>
                <span style={{color:"#66708A",fontWeight:600}}>{i+1}.</span>
                <input
                  value={editAns[i] != null ? editAns[i] : ""}
                  onChange={e=> setEditAns(prev=>({...prev, [i]: e.target.value}))}
                  style={{width:54,padding:"4px 6px",border:"1px solid rgba(15,26,46,.25)",borderRadius:4,fontFamily:"inherit",fontSize:12,textAlign:"center"}}
                />
              </label>
            ))}
          </div>
          <div style={{display:"flex",justifyContent:"flex-end",gap:8,marginTop:10}}>
            <button onClick={()=>setEditing(false)} disabled={savingEdit} style={{...mkBtn("transparent","#66708A"),border:"1px solid rgba(15,26,46,.2)",padding:"5px 12px",fontSize:11}}>Cancel</button>
            <button onClick={saveEdits} disabled={savingEdit} style={{...mkBtn("#003258","#fff"),padding:"5px 14px",fontSize:11,fontWeight:600,cursor:savingEdit?"not-allowed":"pointer"}}>
              {savingEdit ? "Saving + regrading…" : "Save & re-grade"}
            </button>
          </div>
        </div>
      )}
      ...
      {/* Action row: edit answers + re-grade + reset */}
      <div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:8,paddingTop:8,borderTop:"1px dashed rgba(15,26,46,.1)"}}>
        {canEditAnswers && !editing && (
          <button onClick={startEdit} style={{...mkBtn("transparent","#003258"),border:"1px solid rgba(0,50,88,.4)",padding:"5px 12px",fontSize:11}}>
            Edit answers
          </button>
        )}
