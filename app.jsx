...
function AppInner({authUser, onSignOut, currentUserEntry}){
  ...
  // ── Session 18C: Consultation Student seed (+ refresh on version bump) ──
  // Seeds the synthetic demo profile if missing. Also rewrites it when
  // CONSULTATION_DATA_VERSION advances past whatever's stored on the doc,
  // so we can refine the demo data without manual cleanup. Once seeded
  // per session, _consultationSeeded prevents loops.
  ...
  useEffect(()=>{
    ...
    const col = studentsCollection();
    if(!col) return;
    // Already-seeded path: only rewrite if the stored data version is
    // stale. This lets us refine synthetic data in subsequent commits
    // without overwriting on every page load.
    if(existing) {
      const storedVer = Number(existing._dataVersion || 0);
      if(storedVer >= CONSULTATION_DATA_VERSION) {
