...
async function parseWelledReport(file){
  ...
  // Total score
  const totalMatch = textForSearch.match(/TOTAL\s*SCORE\s+(\d{3,4})/i);
  if(totalMatch) result.totalScore = parseInt(totalMatch[1]);

  // Section scores — accept 'and' or '&'
  const rwMatch = textForSearch.match(/Reading\s*(?:and|&)\s*Writing\s+(\d{3})/i);
  if(rwMatch) result.rwScore = parseInt(rwMatch[1]);
  const mathMatch = textForSearch.match(/\bMath\s+(\d{3})\b/i);
...
function AppInner({authUser, onSignOut, currentUserEntry}){
  ...
  const delScore=(sid)=>{
    ...
    if(!confirm(`Remove "${label}"? (Soft-delete — recoverable from Firestore.)`)) return;
    const upd=students.map(st=>st.id===profile.id?{...st,scores:(st.scores||[]).map(sc=>sc.id===sid?softDel(sc):sc)}:st);
    setStudents(upd);
    ...
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"340px 1fr",gap:20}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:20}}>
          ...
          {(p.scores||[]).filter(sc=>!sc.deleted).length>0 && <div style={{...CARD,padding:20}}>
